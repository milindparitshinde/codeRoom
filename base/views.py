...
#     {'id': 2, 'name': "Let's learn ReactJS!"},
#     {'id': 3, 'name': "Let's learn AI!"},
# ]
...
def signupUser(request):
    ...
    return render(request, 'base/login_register.html', {'form': form, 'page': 'signup'})

def home(request):
    q = request.GET.get('q') if request.GET.get('q') != None else ''
    
    if q.startswith('@'):
        rooms = Room.objects.filter(Q(host__username__icontains=q[1:]))
        room_messages = Messages.objects.filter(Q(user__username__icontains=q[1:]))
    else:
...
def updateUser(request):
    ...
    if request.method == 'POST':
        form = UserForm(request.POST, request.FILES, instance=user)  # Include request.FILES
        if form.is_valid():
            form.save()
